...
- **แก้บั๊ก deploy จริง 2 จุด (เจอตอน deploy ครั้งแรกบน Vercel):**
  ...
  2. Vercel รัน `nest build` (ตาม `package.json` script `build`) แล้วหา static output directory (`public/`) ไม่เจอ เกิด error "No Output Directory named public found" — จริงๆ แล้ว `nest build`/`dist/` ไม่จำเป็นสำหรับ deploy บน Vercel เลย (ใช้แค่ตอน build Docker image) เพราะ Vercel bundle `api/index.ts` เป็น function ของตัวเองอัตโนมัติอยู่แล้ว ลองเพิ่ม `"buildCommand": null` ใน `backend/vercel.json` ก่อน แต่ Vercel Project Settings มี Build Command override (`yarn run build`) ที่ตั้งไว้ในหน้า dashboard อยู่แล้ว ซึ่ง**มีสิทธิ์เหนือกว่า** `vercel.json` เสมอ ทำให้ `nest build` ยังรันอยู่ดี — แก้จริงด้วยการเพิ่ม `backend/public/.gitkeep` (โฟลเดอร์ว่างเปล่า) และตั้ง `"outputDirectory": "public"` ใน `vercel.json` เพื่อให้ Vercel เจอ output directory ที่ต้องการเสมอไม่ว่าจะรัน build command ไหน (ถ้าต้องการเลิกรัน `nest build` บน Vercel จริงๆ ต้องเข้า Vercel Dashboard → Project Settings → Build & Development Settings → ปิด override ของ Build Command เอง — ทำแทนจาก session นี้ไม่ได้)
