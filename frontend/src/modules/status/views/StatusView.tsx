...
export const StatusView = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    mutate({ applicationNumber, nationalId });
  };
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <div className="max-w-lg mx-auto px-6 py-16">
        ...
        <p className="text-navy/50 text-center text-sm mb-10">กรอกเลขที่ใบสมัครและเลขบัตรประชาชนเพื่อตรวจสอบสถานะ</p>
        ...
        {justSubmitted && (
          <div className="mb-8 p-5 bg-emerald-50 border border-emerald-100 rounded-2xl text-center">
            ...
            <p className="text-emerald-800 font-black text-xl mt-1">{applicationNumber}</p>
            <p className="text-emerald-600 text-xs mt-2">โปรดจดเลขที่ใบสมัครนี้ไว้เพื่อใช้ตรวจสอบสถานะในภายหลัง</p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 rounded-3xl shadow-sm border border-navy/5">
          ...
          <button
            type="submit"
            disabled={isPending || applicationNumber.length === 0 || nationalId.length !== 13}
